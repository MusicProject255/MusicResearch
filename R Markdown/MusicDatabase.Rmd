...
##Code used to convert json of Gracenote data
```{r eval=FALSE}
library(jsonlite)
library(dplyr)
library(tidyr)
...
tbl_df(gracenote)
# The data frame is not in a 'tidy' format. Several columns are actually nested data frames.
# Use the flatten() function to fix this.
gnTryThis <- flatten(gracenote)
# The 'tracks' column is a list. We could fix this, or we could just take out the column.
# We took the column out, because it's useless for our purposes.
gnTryThis <- gnTryThis %>% select(-tracks)
write.csv(gnTryThis, "gnMetadata5000.csv")
```
